...
export default {
  data() {
    return {
      ...
      activeIndex: "1-1", // 默认激活第一个菜单项
      renderError: false // 渲染错误标志
    };
  },
  created() {
    // 设置默认激活菜单项
  ...
  methods: {
    ...
    setActiveMenu() {
      ...
      if (path.includes('/ucenter/basic')) {
        this.activeIndex = '1-1';
      } else if (path.includes('/ucenter/password')) {
        this.activeIndex = '1-2';
      } else if (path.includes('/ucenter/collect')) {
        this.activeIndex = '1-3';
      } else if (path.includes('/ucenter/study')) {
        this.activeIndex = '1-4';
      }
    },

    getAvatarUrl(avatar) {
      if (!avatar) return require('~/assets/img/default-tea-img.gif');
    ...
    initMotto() {
      try {
        ...
        console.error('初始化励志话语失败:', error);
        this.currentMotto = this.mottoList[0];
      }
    },
    ...
    handSearch() {
      /*if (this.keyword && this.keyword.trim()) {
        this.$router.push({ path: '/course', query: { keyword: this.keyword.trim() } });
      }*/
      this.showQQTip();
    }
  },
...
/* 移除el-menu-item-group的横线 */
.elMenu .el-menu-item-group__title {
  padding: 0 !important;
...
/* 移除el-menu-item-group的边框 */
.el-menu-item-group {
  border: none !important;
}

/* 移除el-menu-item-group的伪元素 */
.el-menu-item-group::before {
  display: none !important;
}
